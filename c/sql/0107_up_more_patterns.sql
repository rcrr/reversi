...
--
-- This is a persistent change that is not rolled-back running the 0107_down_more_patterns.sql script
-- INTEGER index value are used instead of the SMALLINT type.
-- This is required to cope with large patterns like XEDGE (ten squares).
--
BEGIN;
--
-- Populates the table regab_prng_pattern_ranges with the data appropriate for the function argument.
--
CREATE OR REPLACE FUNCTION populate_regab_prng_pattern_ranges (patter_name_arg CHAR(6))
RETURNS INTEGER
AS $$
DECLARE
  nrecords INTEGER;
...
ALTER FUNCTION regab_transformed_pattern_to_index PARALLEL SAFE;
ALTER FUNCTION regab_index_to_transformed_pattern PARALLEL SAFE;
...
---
--- Add column pattern_name_id, populate it, and add a UNIQUE and NOT NULL constraint.
--- Patterns are enumarated exactly as the board_pattern_id_t enum as defined in board_pattern.h
---
--ALTER TABLE regab_prng_patterns ADD COLUMN pattern_name_id SMALLINT;

--UPDATE regab_prng_patterns SET pattern_name_id =  0 WHERE pattern_name = 'EDGE';
--UPDATE regab_prng_patterns SET pattern_name_id =  1 WHERE pattern_name = 'CORNER';
--UPDATE regab_prng_patterns SET pattern_name_id =  2 WHERE pattern_name = 'XEDGE';
--UPDATE regab_prng_patterns SET pattern_name_id =  3 WHERE pattern_name = 'R2';
--UPDATE regab_prng_patterns SET pattern_name_id =  4 WHERE pattern_name = 'R3';
--UPDATE regab_prng_patterns SET pattern_name_id =  5 WHERE pattern_name = 'R4';
--UPDATE regab_prng_patterns SET pattern_name_id =  6 WHERE pattern_name = 'DIAG4';
--UPDATE regab_prng_patterns SET pattern_name_id =  7 WHERE pattern_name = 'DIAG5';
--UPDATE regab_prng_patterns SET pattern_name_id =  8 WHERE pattern_name = 'DIAG6';
--UPDATE regab_prng_patterns SET pattern_name_id =  9 WHERE pattern_name = 'DIAG7';
--UPDATE regab_prng_patterns SET pattern_name_id = 10 WHERE pattern_name = 'DIAG8';
--UPDATE regab_prng_patterns SET pattern_name_id = 11 WHERE pattern_name = '2X5COR';

--ALTER TABLE regab_prng_patterns ALTER COLUMN pattern_name_id SET NOT NULL;

--ALTER TABLE regab_prng_patterns ADD CONSTRAINT regab_prng_patterns_name_id_key UNIQUE (pattern_name_id);


--
-- Renames the function to a consistent naming convention.
--
ALTER FUNCTION populate_regab_prng_pattern_ranges RENAME TO ragab_populate_pattern_ranges;
