...
BEGIN
  ...
  IF (status_cardinality IS NOT NULL AND empty_count_arg >= 0 AND empty_count_arg <= 60) THEN
    ...
    FOREACH pattern_id_current IN ARRAY pattern_id_array_arg LOOP
      ...
      END LOOP;
    END LOOP;
    query_command := format('%s FROM regab_prng_gp AS gp RIGHT JOIN regab_prng_gp_pattern_class AS pc ON gp.seq = pc.gp_id WHERE (gp.legal_move_count_adjusted > 0 OR %s = 0) AND gp.empty_count = %s', query_command, empty_count_arg, empty_count_arg);
    ...
    SELECT count(1) INTO row_cnt FROM regab_prng_gp AS gp RIGHT JOIN regab_prng_gp_pattern_class AS pc ON gp.seq = pc.gp_id
      WHERE (gp.legal_move_count_adjusted > 0 OR empty_count_arg = 0) AND gp.empty_count = empty_count_arg AND gp.status = ANY (status_array_arg) AND gp.batch_id = ANY (batch_id_array_arg);
