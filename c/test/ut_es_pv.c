                                              ...
                                              const endgame_solver_env_t *const env))
{
  ...
  for (int i = 0;; i++) {
    ...
    if (ut_run_time_is_verbose(t)) {
      ...
      fprintf(stdout, "Test #%3d: data[position=%s, expected_value=%+03d, expected_best_moves={%s}]; ", i, tc->gpdb_label, tc->outcome, moves_to_s);
    }

    GamePositionX *const gpx = get_gpx_from_db(t, db, tc->gpdb_label);
    ...
    if (!ok_value || !ok_move || !ok_pv) ut_test_fail(t);

    if (ut_run_time_is_verbose(t)) {
      fprintf(stdout, "result[outcome=%+03d, move=%s, sha=%s]", solution->outcome, square_to_string(solution->best_move), pv_digest_as_string);
