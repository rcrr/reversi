 ...
 *
 * @details The stack uses 88,224 bytes of memory.
 */
typedef struct {
  NodeInfo  *active_node;                                       /**< @brief The active node on the stack. */
  NodeInfo   nodes[GAME_TREE_MAX_DEPTH];                        /**< @brief The stack of node info. */
  gts_mle_t  legal_move_array[MAX_LEGAL_MOVE_STACK_COUNT];      /**< @brief The array hosting the legal moves for each node. */
  gts_mle_t *legal_move_stack[MAX_LEGAL_MOVE_STACK_COUNT];      /**< @brief The stack hosting the legal moves for each node. */
