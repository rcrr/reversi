 ...
 * @copyright 2013, 2014, 2016, 2017, 2021, 2022 Roberto Corradini. All rights reserved.
 *
 * @par License
 * <tt>
...
/*
 * Comment the following line to avoid using the kogge_stone ASM module.
 * The code generated by the two internal C routines is almost equivalent.
 * Performances are on par.
 */
//#define KOST
...
/*
static SquareSet
kogge_stone_lms (const SquareSet generator,
                 const SquareSet propagator,
                 const SquareSet blocker);

static SquareSet
kogge_stone_mm (const SquareSet generator,
                const SquareSet propagator,
                const SquareSet blocker);
*/
...
#else
    result = kogge_stone_lms(m_set, o_set, e_set);
#endif
 ...
 *  - blocker    : mover
 *
 * and returns the bitboard having the flipped set
 *
 ...
 *
 *  - added to the player square set plus the move
                ...
                const SquareSet blocker)
{
  ...
  for (int i = 0; i < 8; i++) {
    ...
    gt = blocker & mask[i] & ((g << slide_up_1[i]) >> slide_dw_1[i]);
