...
/**
 * @brief LegalMoveList structure constructor.
 *
 * An assertion checks that the received pointer to the allocated
 * structure is not `NULL`.
 *
 * @invariant Parameter `legal_move_set` cannot have more then 32 moves.
 * The invariant is guarded by an assertion.
 *
 * @param [in] legal_move_set the set of legal moves
 * @return                    a pointer to a new legal move list structure
 */
LegalMoveList *
legal_move_list_new (const SquareSet legal_move_set)
{
  ...
  static const size_t size_of_legal_move_list = sizeof(LegalMoveList);

  legal_move_list = (LegalMoveList*) malloc(size_of_legal_move_list);
  g_assert(legal_move_list);
  ...
  g_assert(legal_move_list->move_count <= 32);
  
  return legal_move_list;
}

/**
 * @brief LegalMoveList structure destructor.
 *
 * @invariant Parameter `list` cannot be `NULL`.
 * The invariant is guarded by an assertion.
 *
 * @param [in] list the pointer to be deallocated
 * @return          always the NULL pointer
 */
LegalMoveList *
