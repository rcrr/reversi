      ...
      char *argv[])
{
  ...
  /*
   * See document: 20190203_REVERSI_Appunti_calcolo_pesi_glm.pdf
   */

  double *v, *w, *e, *de, *r, *minus_grad_f, **big_b, *aux_diag_big_b, *p, effe, lambda, *big_b_diag;
  ...
  /* big_b_diag: diagonal of big_b matrix used by the Levemberg-Marquardt algorith. */
  big_b_diag = chol_allocate_vector(enne);
  if (!big_b_diag) abort();
  ...
  for (size_t iter = 0; iter < 30; iter++) {
    ...
    printf("effe = %f\n", effe);
    for (size_t i = 0; i < enne; i++) big_b_diag[i] = big_b[i][i];

    for (size_t i = 0; i < enne; i++) big_b[i][i] += lambda * big_b_diag[i];
  ...
  /*
  // START OF THE BUGGY-WAY

  const size_t nk = data.pattern_freq_summary.ntuples;
  double *p, *w, *w_delta, *f, **jacobian;

  w_delta = chol_allocate_vector(nk);
  p = chol_allocate_vector(nk);
  w = chol_allocate_vector(nk);
  f = chol_allocate_vector(nk);
  jacobian = chol_allocate_square_matrix(nk);

  chol_zero_vector(w, nk);

  for (size_t iter = 0; iter < 5; iter++) {

    rglmut_dedwk_equation(&result, &data, f, w, jacobian);

    chol_fact_naive(jacobian, nk, p);
    ...
    chol_solv_naive(jacobian, nk, p, f, w_delta);

    printf("********* iter = %zu **********\n", iter);
    for (size_t k = 0; k < nk; k++) {
      printf("w[%02zu]=%0+20.18f, w_delta=%0+20.18f, f=%0+20.18f\n", k, w[k], w_delta[k], -f[k]);
    }
  ...
  chol_free_matrix(jacobian);
  chol_free_vector(f);
  chol_free_vector(w);
  chol_free_vector(p);
  chol_free_vector(w_delta);

  // END OF THE BUGGY-WAY
  */
