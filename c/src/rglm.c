      ...
      char *argv[])
{
  ...
  if (s_flag) {
    ...
    aux_diag_big_b = lial_allocate_vector(enne);
    ...
    for (size_t iter = 0; iter < max_newton_iter; iter++) {
      ...
      lial_chol_fact_naive(big_b, enne, aux_diag_big_b);
...
#define CHO true
      ...
      lial_dump_matrix(big_b, enne, enne, cho_matrix_file_name, &cho_ret_code);
      if (cho_ret_code != 0) abort();
      printf("CHO - Hessian matrix dumped to binary file: \"%s\".\n", cho_matrix_file_name);
      lial_dump_vector(aux_diag_big_b, enne, cho_diagonal_file_name, &cho_ret_code);
      if (cho_ret_code != 0) abort();
      ...
      lial_chol_solv_naive(big_b, enne, aux_diag_big_b, minus_grad_f, p);
    ...
    lial_free_vector(p);
    lial_free_vector(aux_diag_big_b);
