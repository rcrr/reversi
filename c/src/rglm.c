 ...
 *
 * @todo CSV files dumped from the program are somehow redundant and not well organized.
 *       Moreover the FATURES are not dumped ...
 *
 * @todo Add the intercept parameter w_0
 *       Add the new concept of feature.
 *       Add to features the legal_move_count as MOBILITY and INTERCEPT as the w_0 intercept parameter.
 ...
 *       - w is the vector of weights as computed by solving the GLM system.
 *       - h is the count [0,1,..n_instances] of the occurrences of the pattern configuration in the game position.
 *       <br>
 ...
 *       To better describe this characteristic of the data each position is given a feature that is
 *       always present once, the expected value of the parameter w_0 should then be equal to the mean of the distribution
 *       of the outcomes.
 *       [ 2020-06-02 ] The option to add 'fake' patterns to the model, for w_0 and for additional features like the mobility
 *                      is not very promising.
 *                      A better option is to shift the glm_variable_id by the count of the additional weights. By one if
 *                      only w_0 is added, by more if more features are added.
 *                      In this way we have that w_0 has glm_variable_id = 0, and features comes before patterns.
 *                      REGAB has to be modified, by inserting the count of added features.
 *                      The SQL procedure regab_action_extract_count_pattern_freqs has to be modified, having one more input parameter,
 *                      and shifting the numbering of glm_variable_id accordingly.
 *                      REGAB has to output a table with the added covariates and parameters.
 *                      The binary file format generated by REGAB and read by RGLM has to be changed in order to collect these additional information.
 *                      RGLM has to be adapted in order to read properly the new binary file format.
 *                      <br>
 *                      [ 2020-06-05 ]
 *                      The shift variable has been added to the do_action_extract_pattern_freqs_prepare_cursor function in the regab.c file.
 *                      I am going to add a new FEATURE section in the binary file format.
 *                      A FEATURE is a real variable ( double precision ) assigned to all the positions in the file.
 *                      Associated to each FEATURE there is a one-to-one related weight.
 *                      The count of all FEATURES is then assigned to the glm_variable_id_offset variable.
 *                      All the models have a minimum of one feature named INTERCEPT, being the w_0 regression intercept.
 *                      - Changed the documentation of binary file in rglm_data_files.h
 *                      - Added the enum board_feature_id_t in file board_pattern.h
 *                      - Changed the rglmdf_general_data_t definition in rglm_data_files.h. Added two fields: 'feature_cnt' and 'features'.
 *                      - Changed rglmdf_general_data_init in rglm_data_files.c. The new fields are initialized.
 *                      - Changed rglmdf_general_data_release in rglm_data_files.c. The 'features' array is freed.
 *                      - Added functions: 'rglmdf_set_feature_cnt', 'rglmdf_get_feature_cnt', 'rglmdf_get_features',
 *                        'rglmdf_features_to_text_stream' to rglm_data_files.c and rglm_data_files.h.
 *                      - Added board_feature_t definition in file board_pattern.h , added also array board_features[].
 *                      - [ 2020-06-06 ]
 *                      - Changing the REGAB program.
 *                      - Added section 01.c in the extract action. It checks the selected features.
 *                      - Added the f flag to the REGAB program.
 *                      - Added board_feature_get_id_by_name function definition and implementation in the board_pattern module.
 *                      - Added function do_action_extract_check_features in regab.c
 *                      - Added code in REGAB program, action extract, step 03 to write to the header of the binary file
 *                        the section that lists the features.
 *                      - Added section for reading the feature list from the binary file in RGLM.
 *                      - In function rglmdf_write_rglm_weights_to_binary_file, added section for writing the feature list.
 *                      - In function rglmdf_write_general_data_to_binary_file, added section for writing the feature list.
 *                      - Change: the option -g, --game-positions, in the REGAB program is not compatible with option -f as well as option -g.
 *                      - Change in program RGLM_FIT_UTILITY, added section for reading the feature list from the weights binary file.
 *                      - Change in program RGLM_FIT_UTILITY, added section for reading the feature count from the positions binary file.
 *                      - [ 2020-06-07 ] Moving on from git commit f30e269d51ad7306b77c79e653779237f8ed6972 , starting to work on feature behavior
 *                      - Added two new fields to the rglmdf_pattern_freq_summary_table_s structure ...g
 *                        'size_t glm_f_variable_cnt' : Count of GLM independent variables belonging to features.
 *                        'size_t glm_p_variable_cnt' : Count of GLM independent variables belonging to patterns.
 *                        Changed the documentation of binary file in rglm_data_files.h
 *                        Changed rglmdf_general_data_init in rglm_data_files.c. The new fields are initialized.
 *                        Added functions: 'rglmdf_get_glm_f_variable_cnt', 'rglmdf_get_glm_p_variable_cnt'.
 *                        Changing the REGAB program (many changes).
 *                        Changing RGLM ....
 *                      - [ 2020-06-12 ]
 *                      - RGLM now reads the binari file properly. The solver aborts ....
 *
      ...
      char *argv[])
{
  ...
  if (s_flag || H_flag) {
    ...
    for (size_t iter = 0; iter < max_newton_iter; iter++) {
      ...
      if (false) lial_dpotrf_bp(&chol_upper, &enne_int, *big_b, &enne_int, &ret, bs, tc);
      if (true) lial_dpotrf(&chol_upper, &enne_int, *big_b, &enne_int, &ret);
      if (ret != 0) {
