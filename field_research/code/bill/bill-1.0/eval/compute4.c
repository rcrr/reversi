...
/*
 * compute all the features for a given board, using lots of tables and cycles
 *
     ...
     feature_t features[4];
{
...

  {
    ...
    register int    (*d)[4] = Disp[color],
		    index;
    extern short    Edge_Table[59049];	/* short to save space */
    short	    escore;
    ...
    escore = Edge_Table[index + d[8][b[1][1]] + d[9][b[1][6]]]; /* not += */
    ...
    escore += Edge_Table[index + d[8][b[6][1]] + d[9][b[6][6]]];
    ...
    escore += Edge_Table[index + d[8][b[1][1]] + d[9][b[6][1]]];
    ...
    features[0] = escore + Edge_Table[index + d[8][b[1][6]] + d[9][b[6][6]]];
...
void print_pwm (pwmscore)
     unsigned int pwmscore;
{
  packed mess;

  mess.all = pwmscore;
  printf ("%d %d %d\n",
	  mess.scores.pscore, mess.scores.wscore, mess.scores.mscore);
}
