...
String ClientGame::read_sgf(istream &is)
{
  char c;
  ...
  FOREVER {
	...
	komi_defined = true;
      ...
      if (!start_pos->read_sgf(is).empty()) return "start_pos corrupt";
      ...
      ERR2("unknown SGF command: \a", s);
      return "unknown SGF command";
    }
  }
...
void ClientGame::write_sgf(ostream &os, bool one_line) const
{
  sint4 indb = 0;
  sint4 indw;
  ...
  os << "BO[";
  start_pos->write_sgf(os, one_line);
  os << "]" << my_eol;
...
// 	    ClientColor::WHITE;
  ...
  pinf[pi].clock.stop(tmp_move->get_time());
...
void ClientGame::replay()
{
  ...
  if (type.is_komi_game()) {
    ...
    clock(name_index(name_to_move())).start();
