...
Pattern::Pattern() 
{ 
  tab_v = tab_w = 0; len = 0; trans_num = 0; p_sub = 0; one_phase = false; 
}
...
void Pattern::init(PatternInfo &pinf)
{
  ...
  tab_v = new Entry[Pot3[num]](one_phase);
  if (!tab_v) Error("Pattern::init: no memory");

  tab_w = new Entry[Pot3[num]](one_phase);
  if (!tab_w) Error("Pattern::init: no memory");
  ...
  FOR (i, Pot3[num]) {
    ...
    if (min_s == i) {
      tab_v[min_s].alloc_esti();  // allocate only once!
      tab_w[min_s].alloc_esti();  // allocate only once!      
      allocated++;
    }

    //    if (i == 265020+265720 || i == 265710+265720) { printf("%d %d\n", i-265720, min_s-265720); }

    tab_v[i].p_map = &tab_v[min_s];
    tab_w[i].p_map = &tab_w[min_s];
  ...
  FOR (i, Pot3[num]) {
    if (!tab_v[i].p_map->esti) Error("pv=0");
    if (!tab_w[i].p_map->esti) Error("pw=0");    
  }
...
void Pattern::set_weights(float v)
{
  int i;
  
  FOR (i, Pot3[len]) tab_w[i].p_map->set(v);
}


void Pattern::weight_lower_bound(float v)
{
  int i;
  
  FOR (i, Pot3[len]) tab_w[i].p_map->lower_bound(v);
}


void Pattern::weight_upper_bound(float v)
{
  int i;
  
  FOR (i, Pot3[len]) tab_w[i].p_map->upper_bound(v);
}
...
void Pattern::asc_write(FILE *fp)
{
  ...
  FOR (k, Pot3[len]) {
    ...
    if (one_phase) {
      ...
      fprintf(fp, " %+6.3f/%+6.3f", tab_v[k].p_map->get_y(0), tab_w[k].p_map->get_y(0));
      
    } else {
      
      FOR (i, Entry::INTERVAL_NUM) 
	fprintf(fp, " %+7.1f/%+7.1f", tab_v[k].p_map->get_y(i), tab_w[k].p_map->get_y(i));
...
void Pattern::bin_write(FILE *fp)
{
  ...
  if (one_phase) {
    ...
    FOR (k, Pot3[len]) tab_v[k].bin_write(fp, 0);
    FOR (k, Pot3[len]) tab_w[k].bin_write(fp, 0);
    ...
    FOR (l, Entry::INTERVAL_NUM)
      FOR (k, Pot3[len]) 
        tab_v[k].bin_write(fp, l);

    FOR (l, Entry::INTERVAL_NUM)
      FOR (k, Pot3[len]) 
        tab_w[k].bin_write(fp, l);    
  }
}
...
bool Pattern::bin_read(FILE *fp)
{
  ...
  if (one_phase) {
    ...
    FOR (k, Pot3[len]) tab_v[k].bin_read(fp, 0);
    FOR (k, Pot3[len]) tab_w[k].bin_read(fp, 0);
    ...
    FOR (l, Entry::INTERVAL_NUM)
      FOR (k, Pot3[len]) 
        tab_v[k].bin_read(fp, l);

    FOR (l, Entry::INTERVAL_NUM)
      FOR (k, Pot3[len]) 
        tab_w[k].bin_read(fp, l);    
  }
...
bool Pattern::bin_write_freq(FILE *fp, int n)
{
  ...
  FOR (l, Entry::INTERVAL_NUM)
    FOR (k, Pot3[len]) {
    int val = (tab_v[k].p_map->get_n(l) < n) * 512;
    fputc(val & 255, fp);
    fputc(val >> 8,  fp);
  }
