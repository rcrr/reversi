...
int x;
  ...
  case V_PROB: 
    ...
    sf.Marke = MA_WKEIT + round(100*(1.0/(1.0+exp(-0.1*ValueB))));
    break;
  ...
  if (f_div) {
	...
	fpdiv = fopen("olib_div.sfk", "w");

	if (!fpdiv) Error("can't open olib_div.sfk");
      }
...
error:
  ...
  if (f_write) {
    ...
    FindFreq(fp, pT, pT->pRoot, Game, 0);
  ...
  if (f_alt) {
	...
	for (i=4; i <= 64; i++) {
	  ...
	  FOR (j, 33) {
	    cout << round(Results[i][j]*1000.0/(BoardNum[i])+0.00001) << " ";
	    nums[j] += Results[i][j];
	  }
	  cout << endl;
	}
	...
	FOR (j, 33)
	  cout << round(nums[j]*1000.0/(num+0.00001)) << " ";
	...
	FOR (j, 33) {
	  cout << 
	    round(2*result_distr[j]/(nums[j]*1000.0/(num+0.00001))/min_weight) <<
            ", ";
        }
