...
{
  const int MAX = 1 << 12; char buf[ MAX + 1 ];
  vsprintf( buf, fmt, ap );
  va_end(ap);
  return os << buf;
}
...
sint8 System::real_time()
{
  ...
  if ( ok != 0 ) {
    form(cerr, "gettimeofday( %p, 0 ) = %d", &tv, ok) << endl;
    ERR("");
    return 0;
  }
...
bool System::sock_read( sint4 Sock, IO_Buff& B, uint4 Flag )
{
  ...
  for ( bool first = true ;; first = false ) {
    ...
    if ( no <= 0 ) {
      ...
      form(cerr, "sock_read ::recv( %d, %p, %d, %d ) = %d", Sock, B.begin(), recv_size, Flag, no );
      cerr << endl;
      return false;
    }
    cout.write( B.end(), no );
...
bool System::sock_write( sint4 Sock, IO_Buff& B, uint4 Flag )
{
  ...
  for ( bool first = true ; B.size() ; first = false ) {
    ...
    if ( no <= 0 ) {
      ...
      if ( errno == EINTR  ) return true;
      if ( errno == 0      ) return true; // no error?
      form(cerr, "sock_write ::send( %d, %p, %d ) = %d", Sock, B.begin(), B.size(), no ) << endl;
      return false;
    }
    B.erase( no );
  }
...
bool System::fd_read( sint4 Desc, IO_Buff& B )
{
  ...
  for ( bool first = true ;; first = false ) {
    ...
    if ( no <= 0 ) {
      ...
      if ( errno == EINTR  ) return true;
      if ( errno == 0      ) return true; // no error?
      form(cerr, "fd_read ::read( %d, %p, %d ) = %d", Desc, B.begin(), recv_size, no );
      cerr << endl;;
      return false;
    }
    B.update( no );
...
bool System::fd_write( sint4 Desc, IO_Buff& B )
{
  ...
  for ( bool first = true ; B.size() ; first = false ) {
    ...
    if ( no <= 0 ) {
      ...
      if ( errno == EINTR  ) return true;
      if ( errno == 0      ) return true; // no error?
      form(cerr, "fd_write ::write( %d, %p, %d ) = %d",
	   Desc, B.begin(), B.size(), no ) << endl;
      return false;
    }
    B.erase( no );
  }
...
sint4 System::getsockopt( sint4 Sock, sint4 Level, sint4 Opt, vptr Val, SOCKLEN_T *Len)
{
  ...
  sint4 ok = ::getsockopt( Sock, Level, Opt, Val, (unsigned int *)Len );
...
bool ClientJoin::read(const Client &client, const vector<String>& v)
{
  ...
  istringstream is(s); //.c_str(), s.size());
  ostringstream os;

  // cout << "ggf: " << s << endl;

  game = client.new_ClientGame();
  String es = game->read_ggf(is);
    
  if (!es.empty()) {
    cerr << ">>> corrupt ggf: " << es << endl;
    return false;
  }
...
//  /ams: + match .5 1720 aaa 1720 bbb 4 U
...
// /ams: - match .5 1720 aaa 1720 bbb 4 U +2.00
// /ams: end .6.1 ( bbb vs. aaa ) -1.00 (sent if game ends regularly)

bool ClientEnd::read(const Client &client, const vector<String>& v )
{
  vector<String> vec; String::parse(v[0], vec );
  if (vec.size() < 11) return false;
  if (vec[0] != client.service_login()+":") return false;
  if (vec[1] != "-") return false;
  if (vec[2] != "match") return false;
  id = vec[3];
...
void Client::send( ostringstream& os )
{
  send( os.str() );
...
{
  if ( v[0] != ": + " + client.service_login() + " 1") return false;
...
void Client::io()
{
  ...
  for ( ; type == UNDEF ; ) {
	...
	end.game_in_list1 = false;
	end.game_in_list2 = false;	  
	...
	if ((it=find_game(end.id+".0")) != games.end()) {
	  end.game_in_list1 = true;
	  end.game1 = (*it);
	  games.erase(it);
	}

	if ((it=find_game(end.id+".1")) != games.end()) {
	  end.game_in_list2 = true;
	  end.game2 = (*it);
	  games.erase(it);
	}	

#if DEBUG
	cout << ">>> REMOVED " << (end.game_in_list1 + end.game_in_list2)
	     << " GAME(S) ID=" << end.id << " (# now " << games.size() << ")"
	     << endl;
#endif
	continue;
      }
	...
	if (it != en) {
      ...
      // unknown message: erase & move on

#if DEBUG
      cout << ">>> MESSAGE NOT RECOGNIZED:" << endl;
      FOR (i, mssg.size()) cout << mssg[i] << endl;
#endif
