...
public:
  ...
  inline void new_delta(double ds, int dn)
  {
    delta_sum += ds;
    n += dn;
...
public:
  ...
  enum { DISC_MIN = 13, DISC_MAX = 64,
	 INTERVAL_LEN = 4, INTERVAL_NUM = (DISC_MAX+1-DISC_MIN)/INTERVAL_LEN };
  ...
  inline void new_delta(int inter, double delta, int n) { esti[inter].new_delta(delta, n); }


  // new: update all entries separately

  float update();
  void  regul(float f, int &n, float &sq_sum);
