...
float Entry::update(float &min_val, float &max_val)
{
  int i;
  double max_abs_d = 0;
  ...
  FOR (i, INTERVAL_NUM) {
    if (esti[i].n > 0) {
      float d = esti[i].delta_sum/esti[i].n;
      esti[i].y += d;
...
cout << "(" << i << ": " << esti[i].y << " -> " << new_y << ") " << endl;
  ...
  FOR (i, INTERVAL_NUM) {
    esti[i].delta_sum = 0;
    esti[i].n = 0;
    if (esti[i].y < min_val) min_val = esti[i].y;
    if (esti[i].y > max_val) max_val = esti[i].y;    
  }
...
#define ROUND_FACTOR (32000.0/5000.0)

void Entry::bin_write(FILE *fp, int inter)
{
  int val = int(round(p_map->get_y(inter) * ROUND_FACTOR));
...
void Entry::bin_read(FILE *fp, int inter)
{
  short val = fgetc(fp);
  val += fgetc(fp) << 8;
  p_map->set_y(inter, val / ROUND_FACTOR);
}
...
void Entry::lower_bound(float v)
{
  int n = 1, i;

  if (!one_phase) n = INTERVAL_NUM;

  FOR (i, n) if (esti[i].y < v) esti[i].y = v;
}
  
void Entry::upper_bound(float v)
{
  int n = 1, i;
