/*
   ...
   Modified:       Jan 16, 2022 (Roberto Corradini)
                   December 25, 2005 (last change by G.A.)
...
main( int argc, char *argv[] ) {
  ...
  if ( !tournament && !run_script ) {
    while ( skill[BLACKSQ] < 0 ) {
      ...
      (void) !scanf( "%d", &skill[BLACKSQ] );
      if ( skill[BLACKSQ] > 0 )
        (void) !scanf( "%d %d", &exact_skill[BLACKSQ], &wld_skill[BLACKSQ] );
    }
    while ( skill[WHITESQ] < 0 ) {
      printf( "White parameters: " );
      (void) !scanf( "%d", &skill[WHITESQ] );
      if ( skill[WHITESQ] > 0 )
        (void) !scanf( "%d %d", &exact_skill[WHITESQ], &wld_skill[WHITESQ] );
    }
  }
...
play_game( const char *file_name,
 ...
 START:
  ...
  if ( move_file != NULL ) {
    ...
    (void) !fgets( line_buffer, sizeof line_buffer, move_file );
...
run_endgame_script( const char *in_file_name,
  ...
  for ( i = 0; ; i++ ) {
    ...
    (void) !fgets( buffer, BUFFER_SIZE, script_stream );
