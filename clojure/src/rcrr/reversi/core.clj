...
(ns rcrr.reversi.core
  (:use [clojure.test :only (is are run-tests)])
  (:require [rcrr.auxfns :as auxfns]
...
(defn
  #^{:doc "Query a board for a given square."
     :test (fn []
             (is (= (board-ref fixt/initial-board 0) outer)
                 "Every board has the 0 square being outer.")
             (is (= (board-ref fixt/initial-board 100) nil)
                 "An index out of the square vector returns nil.")
             (is (= (board-ref fixt/initial-board 11) empty-square)
                 "Initial board has a1 square empty.")
             (is (= (board-ref fixt/initial-board 45) black)
                 "Initial board has e4 square with a black disc.")
             (is (= (board-ref fixt/initial-board 44) white)
                 "Initial board has d4 square with a white disc."))}
...
(defn
  #^{:doc "Set the value of a given board square pairs.
     ...
     :test (fn []
             (let [sq 11
                   player black]
               (is (= (board-ref (board-set fixt/empty-board sq player) sq) player)
...
(defn
  #^{:doc "Create a new copy of the given board."
     :test (fn []
             (is (= fixt/initial-board (copy-board fixt/initial-board)) 
  ...
  #^{:doc "Return a board, empty except for four pieced in the middle."
     :test (fn []
             (let [b (initial-board)]
               ...
               (is (= (initial-board) fixt/initial-board))))}
  initial-board []
  ...
  #^{:doc "Count player's pieces minus opponent's pieces."
     :test (fn []
             ...
             (is (= (count-difference black fixt/board-end-game-x) 10) 
                 "on board-end-game-x board black has 10 pieces 
                  more than white.")
             (is (= (count-difference white fixt/board-end-game-x) -10) 
                 "on board-end-game-x board white has 10 pieces 
                  less than black.")
...
(defn
  ...
  time-string [time]
  (let [t (clojure.math.numeric-tower/round (/ time internal-time-units-per-second))
        min (quot t 60)
        sec (rem t 60)]
  ...
  #^{:doc "Return a new board to reflect move by player."
     :test (fn []
             (is (= (make-move 34 black (initial-board)) fixt/board-a)
                 "black does first move to 34, board-a is the resulting board."))}
  ...
  #^{:doc "Does player have any legal moves in this position?"
     :test (fn []
             ...
             (is (not (any-legal-move? black fixt/board-black-has-to-pass))
                 "black has no legal moves given the board board-black-has-to-pass.")
             (is (not (not (any-legal-move? white fixt/board-black-has-to-pass)))
                 "white has some legal moves given the board board-black-has-to-pass.")
             (is (not (any-legal-move? black fixt/board-end-game-x))
                 "given board board-end-game-x the game is over, black has no move.")
             (is (not (any-legal-move? white fixt/board-end-game-x))
                 "given board board-end-game-x the game is over, white has no move."))}
...
(defn
  #^{:doc "Compute the player to move next, or nil if nobady can move."
     :test (fn []
             (is (= (next-to-play fixt/initial-board white false) black)
...
(defn
  #^{:doc "Returns a list of legal moves for player."
     :test (fn []
             (is (= (legal-moves black fixt/initial-board) '(34 43 56 65))
...
(defn
  #^{:doc "Call the player's strategy function to get move.
     ...
     :test (fn []
             (dotimes [i 10]
               (let [[board move clock] (get-move (fn [_ _] (auxfns/rand-elt (range 0 100)))
  ...
  (let [t0 (auxfns/get-internal-real-time)
        move (strategy player (copy-board board))
        t1 (auxfns/get-internal-real-time)
        delta-t (- t1 t0)
...
(defn
  ...
  (rand-nth (legal-moves player board)))
  ...
  (let [bl-strategy random-strategy
    ...
    (try
      (loop [board (initial-board)
             ...
             player black]
        (if-not player
          (throw (new GameOverException {:game-over-value (count-difference black board)
                                         :board board
                                         :moves moves}
                      "No more moves left."))
      ...
      (catch GameOverException goe
             [(:game-over-value @goe) (:board @goe) (reverse (:moves @goe))]))))
      ...
      (catch GameOverException goe 
             (do
               (when print
                 ...
                 (print-board (:board @goe) (:clock @goe))
                 (println "Game moves: " (map conv-88->h8 (reverse (:moves @goe)))))
               (:game-over-value @goe))))))
...
(defn
  #^{:doc "Given a MOVES list, a BOARD, and the PLAYER that has to move,
           ...
           one or more moves."
     :test (fn []
             (is (= (play-moves fixt/game-x fixt/initial-board black) fixt/board-end-game-x)
                 "Following move by move the game-x sequence should result 
                  into the board-end-game-x final board."))}
