...
(ns rcrr.reversi.strategies
  (:use rcrr.reversi.core
        rcrr.reversi.edge-table-utils
        [clojure.test :only (is are run-tests)])
  (:require [clojure.math.numeric-tower :as math]
            [rcrr.auxfns :as auxfns]
  ...
  #^{:doc "Return a strategy that will consider every legal move,
     ...
     :test (fn []
             (let [weights fixt/weights-1
                   ...
                   value (tws p b)
                   move (max-tws p b)
                   next-value (tws p (make-move move p b))]
               (is (= value -1))
               (is (= move 47))
               (is (= next-value 2))))}
  maximizer [eval-fn]
...
(defn
  #^{:doc "Is tihs a win, loss, or a draw for player?
     ...
     :test (fn []
             ...
             (is (= (final-value black fixt/initial-board) 0))
             (is (= (final-value white fixt/initial-board) 0)))}
  final-value [player board]
  (let [cd (count-difference player board)]
    (condp = (Integer/signum cd)
      -1 (+ losing-value 0)
      0 0
      +1 (+ winning-value 0))))
...
(def ^:dynamic *all-squares-static-ordered*
...
(defn
  #^{:doc "Returns an ordered list of legal moves for player."
     :test (fn []
             (is (= (legal-moves-optimized black fixt/initial-board) '(34 43 56 65))
